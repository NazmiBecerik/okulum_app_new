...
import 'package:okulum_app/app/modules/common/controllers/route_controller.dart';
import 'package:okulum_app/generated/locales.g.dart';
...
extension CustomGetDialogExtension on GetInterface {
  ...
  Future showAwesomeDialog({
    ...
    if (context != null) {
      ...
      Widget buildButton({required String text, required Color textColor, required Color backgroundColor, required Function()? onPressed}) =>
          CustomInkwell(
            onTap: onPressed,
            padding: EdgeInsets.zero,
            child: Container(
              decoration: BoxDecoration(color: backgroundColor, borderRadius: BorderRadius.circular(Utils.normalRadius)),
        ...
        btnOk: disableBtnOk
            ...
            : StatefulBuilder(
                builder: (context, setState) {
                  if (counter > 0) {
                    cancellableOperation = CancelableOperation.fromFuture(Future.delayed(const Duration(seconds: 1)), onCancel: () => {});
      ...
      ).show().whenComplete(() => cancellableOperation?.cancel());
    }
  }
