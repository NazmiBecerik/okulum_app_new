...
import 'package:okulum_app/app/modules/common/widgets/dialog/awesome_dialog/custom_verticalstackdialog.dart';
...
class CustomAwesomeDialog {
  ...
  }) : assert(autoDismiss || onDissmissCallback != null, 'If autoDismiss is false, you must provide an onDissmissCallback to pop the dialog');
  ...
  Future<dynamic> show() => showDialog(
    ...
    builder: (BuildContext context) {
      if (autoHide != null) {
        Future<void>.delayed(autoHide!).then((dynamic value) => _onDissmissCallbackCalled ? null : dismiss());
      }
      switch (animType) {
        case AnimType.SCALE:
          return ScaleFade(scale: 0.1, curve: Curves.fastLinearToSlowEaseIn, child: _buildDialog);
  ...
  )..then((dynamic value) => _onDissmissCallbackCalled ? null : onDissmissCallback?.call(_dismissType));
  ...
  Widget? get _buildHeader {
    ...
    return FlareHeader(loop: headerAnimationLoop, dialogType: dialogType);
  }
